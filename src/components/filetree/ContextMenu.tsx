...
const itemStyle: React.CSSProperties = { width: '100%', textAlign: 'left', padding: '6px 14px', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12 };
...
export function ContextMenu({ menu, onClose, onAction }: {
  ...
  const isSearch = (menu as any).extra === 'search';
  const items = isSearch
    ? [{ id: 'reveal', label: 'Reveal in file tree' }, null, { id: 'copyPath', label: 'Copy Path' }]
    ...
    <div ref={ref} style={{ position: 'fixed', top: menu.y, left: menu.x, zIndex: 9999, background: 'var(--bg-4)', border: '1px solid var(--border-2)', borderRadius: 8, minWidth: 160, boxShadow: '0 8px 24px rgba(0,0,0,0.6)', padding: '4px 0' }}>
      {items.map((item, i) => item === null
        ...
        : <button key={item.id} onClick={() => { onAction(item.id, menu.entry); onClose(); }}
            style={{ ...itemStyle, color: (item as any).danger ? 'var(--red)' : 'var(--text-2)' }}
