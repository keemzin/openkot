---
name: qurandev
description: Fetch word-by-word breakdown + 1 tafsir (Ma'arif-ul-Quran) for a Quran verse, plus memorization guide (sentence-based chunking, context, visual imagery). Simplified, verse-focused output. Accepts "chapter:verse" (e.g., 55:33) or "Surah Name verse" (e.g., Ar-Rahman 33).
agent: general
---
...
"{indonesian_translation}" *(fetch from API with translations=33 or similar ID)*
...
*Example for 55:33:*
1. **يَـٰمَعْشَرَ ٱلۡجِنِّ وَٱلۡإِنسِ** – "O assembly of jinn and humans" (The challenge is issued).
...
## Notes
- Uses `curl` for fast API fetches (no browser needed)
